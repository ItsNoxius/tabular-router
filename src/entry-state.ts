...
export function findEntryInTabs(
    entryId: string
): { tabIndex: number; entryIndex: number } | null {
    for (let tabIndex = 0; tabIndex < routerState.tabs.length; tabIndex++) {
        const entryIndex = routerState.tabs[tabIndex].history.findIndex(
            (e) => e.id === entryId
        );
...
export function setEntryStatePath(
    entry: HistoryEntry,
    path: string[],
    value: unknown
): void {
...
export function getEntryRouteKey<T>(
    entry: HistoryEntry,
    key: string
): T | undefined {
...
export function setEntryRouteKey(
    entry: HistoryEntry,
    key: string,
    value: unknown
): void {
...
export function getEntryNamespace(
    ...
    if (loc) {
        const state =
            routerState.tabs[loc.tabIndex].history[loc.entryIndex].state;

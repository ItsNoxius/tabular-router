...
export function setDocumentTitle(title: string) {
    const tabIndex = routerState.tabs.findIndex(
        (t) => t.id === routerState.activeTabId
    );
    if (tabIndex < 0) return;
    const entry = routerState.tabs[tabIndex].history[
        routerState.tabs[tabIndex].historyIndex
    ];
    if (!entry) return;
...
function snapshotTab(tab: Tab): ClosedTabSnapshot {
    const history = Array.from(tab.history as HistoryEntry[]).map(
        snapshotHistoryEntry
    );
    const windows = Array.from(tab.windows as TabWindowState[]).map(
        snapshotWindow
    );
    return {
        title: tab.title,
...
function cloneSnapshotAsNewTab(snapshot: ClosedTabSnapshot): Tab {
    ...
    const mountedEntryIds = snapshot.mountedEntryIds.map(
...
export function duplicateTab(tabId?: string): boolean {
    ...
    const source =
        tabId != null
            ? routerState.tabs.find((t) => t.id === tabId)
            : getActiveTab();
...
export {
    createEntry, getActiveEntry, getActiveTab, routerState,
    setRouterState
};

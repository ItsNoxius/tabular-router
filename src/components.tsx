...
function CachedEntry(props: {
    ...
    const isActive = createMemo(() => {
        ...
        return (
            activeTab?.id === props.tab.id &&
            activeEntry?.id === props.entryId
        );
    });
        ...
        <For each={routerState.tabs}>
            {(tab) => (
                <For each={tab.mountedEntryIds}>
                    {(entryId) => (
                        <CachedEntry tab={tab} entryId={entryId} routes={routes()} />
                    )}
                </For>
            )}
        </For>
...
function RouterBootstrap(props: {
    rootPath?: string;
    initialPath?: string;
    maxTabs?: number;
}) {
